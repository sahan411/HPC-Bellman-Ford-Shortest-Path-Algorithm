...
The UI runs the compiled binaries in `bin/`, so build the C/CUDA programs first. UI executions use a temporary working directory under `.ui_runs/`, so distance files generated by test runs do not dirty the repository.
